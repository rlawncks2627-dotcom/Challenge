...
- 화면 미리보기: [`docs/preview.html`](docs/preview.html) — 서버 없이 열리는 정적 미리보기
- 관리 도구: [`docs/admin.html`](docs/admin.html) — 더블클릭으로 열어 바로 캠페인을 관리하는 단일 파일
...
`docs/admin.html` 은 배포된 `/admin` 과 같은 RPC 를 직접 부르는 단일 파일이다. 더블클릭으로 열면 인터넷만 있으면 바로 캠페인 설정·실천 항목·참가자 현황을 다룰 수 있다. 배포가 멈췄거나 인터넷 주소를 기억하지 못하는 상황에서도 캠페인을 굴릴 수 있게 두었다.

파일에 담긴 비밀은 없다. Supabase 주소와 publishable 키는 어차피 공개되는 값이고, **관리자 코드는 열 때마다 입력받아 sessionStorage 에만 둔다**(탭을 닫으면 사라진다). 공용 컴퓨터에서 썼다면 작업 후 화면 오른쪽 위 **나가기**를 누른다.

캠페인을 새로 만드는 기능은 넣지 않았다. 생성 코드까지 다루는 화면이 파일로 돌아다니면 위험 범위가 커진다. 캠페인 생성은 배포된 `/admin/new` 에서만 한다.
