...
| 계정 | 초대코드 + 학년·반·번호 + 닉네임 | 가입도 비밀번호도 없이 참가. 2026-08-05 전환 |
...
- `participants` INSERT: 정책 없음. 참가는 `roster_sign_in` RPC(SECURITY DEFINER)로만 가능하고 그 안에서 초대코드를 검증한다. service role 키를 앱에 두지 않기 위한 선택.
...
app/
  ...
  join/[code]/page.tsx          캠페인 확인 + 학년·반·번호 선택 + 닉네임
...
**Phase 3 — 참가 플로우**
초대코드 입력 → 캠페인 확인 → 학년(1~6)·반(1~5)·번호(1~30) 선택 + 닉네임 → 참가자 생성 → `/today` 이동. 닉네임 중복과 잘못된 코드 처리 포함.

**참가자는 계정을 만들지 않는다.** 그런데 RLS 가 이 앱의 유일한 보안 경계이고 그 경계는 `auth.uid()` 위에 서 있다. 그래서 학년·반·번호로 결정되는 내부 계정을 서버가 대신 만들고 로그인시킨다(`roster_sign_in` RPC). 비밀번호는 호출할 때마다 새로 만들어져 서버 액션 안에서만 쓰이고 브라우저로 내려가지 않는다. 내부 계정 주소는 예약 TLD(`.invalid`)를 쓰므로 실제 사용자 계정과 겹칠 수 없다.

같은 자리를 다시 고르면 그 기록으로 이어지고, 그때 정한 닉네임은 새로 입력한 값으로 덮어쓰지 않는다.

**받아들인 약점**: 비밀번호가 없으므로 초대코드를 아는 사람은 같은 반 친구의 번호를 골라 그 기록에 들어갈 수 있다. '비밀번호 없이'를 택하면 피할 수 없다. 초대코드를 캠페인 밖으로 흘리지 않는 것이 유일한 방어선이다.
...
- **단위 테스트 (Vitest, `*.test.ts`)**: CO2 환산, 날짜 경계(서울 자정), 달력 그리드, 초대코드 정규화
- **E2E (Playwright, `e2e/*.spec.ts`)**: 자리 선택으로 참가 → 항목 체크 → 도장 → 순위 반영 / 같은 자리 재참가 시 기록 이어짐 / 사진 첨부 시 브라우저 리사이즈(2000×1500 → 1200×900) → 피드 노출 / 모바일 폭 가로 스크롤 없음 / 404 안내
...
- **초대코드 관리**: 참가에 필요한 비밀은 초대코드 하나뿐이다. 캠페인 밖으로 새면 누구나 아무 자리로 참가할 수 있으니, 학급·부서 안에서만 공유한다.
- **자리 범위**: 학년 1~6 / 반 1~5 / 번호 1~30 으로 고정돼 있다(`lib/roster.ts` 와 DB check 제약). 다른 규모의 조직에 쓰려면 두 곳을 함께 고친다.
